...
class Converter:
    ...
    def open_flags_quiz(self):
        ...
        self.result_label = Label(self.flags_window, text="", fg="#000000")
        self.result_label.grid(row=len(choices) + 4, columnspan=2, padx=10, pady=5)

        # Function to proceed to the next question
        def next_question():
            self.flags_window.destroy()  # Destroy the current question window
            self.open_flags_quiz()  # Open the next question

        # Button to proceed to the next question
        self.next_question_button = Button(self.flags_window, text="Next Question", command=next_question)
        self.next_question_button.grid(row=len(choices) + 5, columnspan=2, padx=10, pady=10)
    ...
    def open_country_quiz(self):
        ...
        self.answer_entry.choices = []  # Country List
        ...
        def check_country_answer():
            ...
            # Function to proceed to the next question
            def next_question():
                self.countries_window.destroy()  # Destroy the current question window
                self.open_country_quiz()  # Open the next question

            # Add button for next question
            next_button = Button(self.countries_window, text="Next Question", command=next_question)
            next_button.grid(row=3, columnspan=2, padx=10, pady=10)
        ...
        # Label to display result message
        result_label = Label(self.countries_window, text="", fg="#000000")
